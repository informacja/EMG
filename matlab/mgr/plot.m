...
cd("/Users/puler/Documents/GitHub/EMG/matlab/mgr/Miopatia zapalna")

% WinFF directory to find FFMPEG.exe
path_ffmpeg = '"C:\Program Files\WinFF\ffmpeg.exe"';
path_ffmpeg = '"/Applications/ffmpeg"';


 [b, folderName] = fileparts(pwd()); sgtitle(folderName)
...
while 1;
	...
	for m = 3:numlist	% skip . & ..
		if dirlist(m).isdir == 0	% file
			if strcmpi(dirlist(m).name(end-2:end),'wav')
...
z = int32(sqrt(numel(a.file_wma)-1));
z=2;
figure(10);
for m = 2:numel(a.file_wma)	% 1st one is blank
    subplot(double(z),double(z),(m-1));
    a.file_wma{m}
    x = audioread(a.file_wma{m});
    plot(x);
