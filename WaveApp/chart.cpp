...
void Chart::drawLogGrid(QPainter &painter, QRect geometry)
{
    ...
    for(int px=0; px<gridNumX; px++){
        ...
        if((px % 9) == 0){
            ...
            painter.drawText(QPointF(gx+logTable[px]*dx-(font.pointSize()), gy+gh+(font.pointSize()*2)), QString().asprintf("%4d",f));
            f*=10;
        }
    }
    painter.drawText(QPointF(gx+logTable[gridNumX]*dx-(font.pointSize()), gy+gh+(font.pointSize()*2)), QString().asprintf("%4d",maxValueX));


    for(int py=0; py<=gridNumY; py++)
        painter.drawText(QPointF(gx-(font.pointSize()*4), gy+(font.pointSize()/2)+py*dy), QString().asprintf("%4d", static_cast<int>(maxValueY-(dvy*py)/gridNumY)));
}
...
void Chart::drawLinearData(QPainter &painter, QVector<double> &data, QColor plotColor)
{
...
void Chart::showMarkers(QPainter &painter, QVector<double> &data)
{
    ...
    if(chartMode==LinearChart){
        ...
        painter.drawText(QPointF(gx+(markerX-minValueX)*dmkx, gy-font.pointSize()), QString().asprintf("%.1f",markerX));
        painter.drawText(QPointF(gx+gw+font.pointSize(), gy+(maxValueY-markerY)*dmky), QString().asprintf("%.1f",markerY));
        ...
        painter.drawText(QPointF(gx+gw+font.pointSize(), gmy-data[static_cast<int>(markerX)-1]*dy), QString().asprintf("%.1f", data[static_cast<int>(markerX)-1]));
    ...
    if(chartMode==SemiLogChart){
        ...
        painter.drawLine(QLineF(gx+((markerX-minValueX)*dmkx), gy, gx+((markerX-minValueX)*dmkx), gy+gh));
        painter.drawLine(QLineF(gx, gy+((maxValueY-markerY)*dmky), gx+gw, gy+((maxValueY-markerY)*dmky)));
        painter.drawText(QPointF(gx+(markerX-minValueX)*dmkx, gy-font.pointSize()), QString().asprintf("%.1f", xlog));
        painter.drawText(QPointF(gx+gw+font.pointSize(), gy+(maxValueY-markerY)*dmky), QString().asprintf("%.1f", markerY));

        pen.setColor(markersColorB);
        painter.setPen(pen);
        painter.drawLine(QLineF(gx, gmy-data[static_cast<int>(xlog)-1]*dy, gx+gw, gmy-data[static_cast<int>(xlog)-1]*dy));
        painter.drawText(QPointF(gx+gw+font.pointSize(), gmy-data[static_cast<int>(xlog)-1]*dy), QString().asprintf("%.1f", data[static_cast<int>(xlog)-1]*180));
    }
    ...
    if(chartMode==LogChart){
        ...
        painter.drawText(QPointF(gx+gw+font.pointSize(), gy+20*log10(1/data[static_cast<int>(xlog)-1])*dy), QString().asprintf("%.1f", -20*log10(1/data[static_cast<int>(xlog)-1])));
    }
}
